...
FILES=" ../_Burtoni_annotations/Astatotilapia_burtoni.BROADAB2fix_CDS.gtf 
		...
		../gatkbp/tophat/ATCACG/accepted_hits.RG.DD.reorderSam.realign.BQSR.bam.SNPs.vcf
		../gatkbp/tophat/CGATGT/accepted_hits.RG.DD.reorderSam.realign.BQSR.bam.SNPs.vcf
	  "
SUBJECTS="3677"	  
...
do
	...
	do 
		...
		echo "    $str"
		bedtools intersect -a $s".wig.bed.nucs_Gflip_CH" -b $f -wo > $s"_"${str}"-overlap"
	done
done